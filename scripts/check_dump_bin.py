...
class CheckBin:
    ...
    def _compare(self, file_path: Path):
        ...
        # Ensure qlib is initialized in separate worker process
        try:
            D.calendar()
        except Exception:
            qlib.init(**self.qlib_init_kwargs)
        ...
        if Compare is None:
            raise ImportError("datacompy.Compare could not be imported. Please ensure datacompy is installed.")
